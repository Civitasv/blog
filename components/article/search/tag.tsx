...
export default function Tag() {
    ...
    const onSearchTagChange = useCallback((e) => {
        //? when content change, refresh search
        const value = e.target.value;
        fetch(`/api/tag/search?tag=${value}`).then(res => {
            return res.json();
        }).then(res => {
    ...
    return (
        <>
            <div className="search cursor-pointer">
                <svg onClick={onTagClick} xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                ...
                <Dialog open={showDialog} onClose={closeModal} as="div" className="fixed flex justify-center inset-0 z-10 overflow-y-auto">
                    ...
                    >
                        <div className="inline-block w-full h-full p-6 text-left transition-all transform bg-white overflow-hidden">
                            ...
                            <div className="pt-2">
                                <input onChange={onSearchTagChange} type="text" name="search" className="w-full h-12 border-4 border-indigo-200 text-lg" />
                            </div>
