...
export default function Tag() {
  ...
  const onSearchTagChange = useCallback((e) => {
    ...
    fetch(`/api/tag/search?tag=${value}`)
      .then((res) => {
        return res.json();
      })
      .then((res) => {
    ...
    <>
      ...
      <Transition appear show={showDialog} as={Fragment}>
        <Dialog
          ...
          onClose={closeModal}
          as="div"
          className="fixed flex justify-center inset-0 z-10 overflow-y-auto"
        >
          <Transition.Child
            as={Fragment}
            ...
            <div className="inline-block w-full h-full p-6 text-left transition-all transform bg-white overflow-hidden">
              ...
              <div className="pt-2">
                <input
                  ...
                  type="text"
                  name="search"
                  className="w-full h-12 border-4 border-indigo-200 text-lg"
                />
              </div>
